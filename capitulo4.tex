...
\chapter{Conexiones de Amari y la Familia $\alpha$}
...
Shun--ichi Amari introdujo una familia uniparamétrica de conexiones, las \textbf{$\alpha$-conexiones}, que unifica bajo un solo marco las tres estructuras geométricas fundamentales: la conexión exponencial ($\alpha = 1$), la conexión de mixture ($\alpha = -1$) y la conexión de Levi--Civita ($\alpha = 0$, la ``media'' de las otras dos).
...
\section{Definición de $\alpha$-conexiones}
...
\section{Dualidad de $\alpha$-conexiones}
...
\section{Tensor de curvatura $\alpha$}
...
plt.plot(t_exp, p_exp, 'b-', linewidth=2, label=r'$\alpha=1$ (exponencial)')
plt.plot(t_mix, p_mix, 'r-', linewidth=2, label=r'$\alpha=-1$ (mixture)')
plt.plot(t_levi, p_levi, 'g--', linewidth=2, label=r'$\alpha=0$ (Levi-Civita)')
plt.xlabel('t')
plt.ylabel('p(t)')
plt.legend()
plt.title('Geodesicas alpha entre Bernoulli(0.2) y Bernoulli(0.8)')
...
for alpha in [-1, 0, 1]:
    ...
    print(f"Gamma^(alpha={alpha:2d})_111(p=0.5) = {g:.4f}")
\end{lstlisting}
...
\begin{enumerate}
	\item Verdadero o falso: La $\alpha$-conexión de Levi--Civita ($\alpha = 0$) es la única conexión compatible con la métrica de Fisher.
	\item ¿Cuál es la relación entre las geodésicas exponenciales ($\alpha = 1$) y las geodésicas de mixture ($\alpha = -1$) en una familia exponencial?
	\item Defina la torsión de una $\alpha$-conexión.  ¿Cuándo se anula?
	\item ¿Por qué las $\alpha$-conexiones son importantes para la inferencia estadística?
	\item Explique la dualidad entre $\alpha$ y $-\alpha$ geodésicas.
\end{enumerate}

\subsection*{Cálculos}
\begin{enumerate}[resume]
	\item Para $\mathrm{Bern}(p)$, calcule los símbolos de Christoffel $\Gamma^{(1)}_{111}$ y $\Gamma^{(-1)}_{111}$.
	\item Para $\mathcal{N}(\mu, \sigma^2)$ con $\sigma^2$ fijo, calcule $\Gamma^{(\alpha)}_{111}$ en función de $\alpha$.
	\item Encuentre la ecuación geodésica $\alpha$ para la familia $\mathrm{Poisson}(\lambda)$.
	\item Para $\mathrm{Bern}(p)$ con $p_0 = 0.3$ y $p_1 = 0.7$, calcule el punto medio de la geodésica $\alpha$ para $\alpha = 0, 1, -1$.
	\item Verifique que $\Gamma^{(0)}_{ijk} = \frac{1}{2}(\Gamma^{(1)}_{ijk} + \Gamma^{(-1)}_{ijk})$ para Bernoulli.
	\item Calcule la curvatura escalar de la métrica de Fisher para $\mathrm{Bern}(p)$.
\end{enumerate}

\subsection*{Teóricos}
\begin{enumerate}[resume]
	\item Demuestre que la torsión de la $\alpha$-conexión es $T^{(\alpha)} = \alpha \cdot T^{(1)}$, donde $T^{(1)}$ es la torsión de la conexión exponencial.
	\item Muestre que en una familia exponencial, las geodésicas $\alpha = 1$ son líneas rectas en las coordenadas naturales $\eta$.
	\item Demuestre que la métrica de Fisher es invariante bajo reparametrizaciones para cualquier valor de $\alpha$.
	\item (Reto) Para la familia de Cauchy con parámetro de localización $\theta$, muestre que la curvatura de la $\alpha$-conexión de Levi--Civita es constante y positiva.
	\item (Reto) Demuestre que las $\alpha$-geodésicas en una familia exponencial satisfacen una ecuación diferencial de tipo Riccati.
\end{enumerate}
