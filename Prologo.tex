...
Este libro parte de una tensión de todos los días: el mundo sigue $p(x)$ (la \textbf{verdad}) y nosotros trabajamos con $q_\theta(x)$ (el \textbf{modelo}, con parámetro $\theta$).
Para evitar ambigüedades, usaremos:
\begin{itemize}
	\item \textbf{Verdad:} $p(x)$.
	\item \textbf{Modelo:} $q_\theta(x)$.
	\item \textbf{Prior:} $\pi(\theta)$.
	\item \textbf{Posterior:} $\pi(\theta\mid x)$.
\end{itemize}

Bayes da una receta de actualización limpia usando el modelo:
\[
\pi(\theta\mid x)\;\propto\;\pi(\theta)\,q_\theta(x).
\]
...
\begin{enumerate}
	...
	\[
	g_{ij}(\theta)\;=\;\mathrm{E}_{\theta}\!\big[\partial_i \log q_\theta(X)\,\partial_j \log q_\theta(X)\big].
	\]
...
Imagina que, tras ver datos nuevos, quieres acercar $q_\theta(x)$ a $p(x)$. Hay dos esquemas muy usados, con interpretación directa en \emph{credibilidad}:
