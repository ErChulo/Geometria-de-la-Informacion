...
\bigskip
\noindent
Antes de definir las conexiones, conviene revisar el idioma geométrico básico que ya usamos en los Capítulos~1--3: una variedad es un conjunto de distribuciones, el espacio tangente $T_\theta \mathcal{M}$ contiene los vectores de velocidades con que podemos movernos infinitesimalmente en $\theta$, y dos curvas distintas ---exponencial y de mixture--- definen dos maneras de ``arrastrar'' vectores a lo largo de una trayectoria. Esa diferencia de arrastres es, precisamente, lo que la familia $\alpha$ parametrizará:
...
\section[Definición de $\alpha$-conexiones]{Definición de $\alpha$-conexiones}
...
\bigskip
\noindent
La dualidad $\alpha \leftrightarrow -\alpha$ que vamos a enunciar es consecuencia directa de cómo se relacionan las dos conexiones extremas en una familia exponencial: la conexión exponencial ($\alpha = 1$) arrastra vectores en coordenadas naturales $\eta$, la de mixture ($\alpha = -1$) los arrastra en coordenadas de expectación $\mu$. Una vez entendida esa correspondencia ---y la torsión que las separa---, el camino natural es preguntarse por las trayectorias mismas: las geodésicas $\alpha$.
...
\bigskip
\noindent
La curvatura que aparece en una $\alpha$-conexión mide algo cuantitativo y útil: cuanto más alta, más ``tuerce'' el espacio al propagar vectores por una curva cerrada. Pero la observación fundamental ---que la única conexión métrica-compatible es Levi--Civita ($\alpha = 0$)--- merece su propia demostración, no suposición silenciosa:

\section[Geodésicas $\alpha$]{Geodésicas $\alpha$}
...
\bigskip
\noindent
Las dos rutinas que acabamos de exhibir ---geodésica $\alpha$ en Bernoulli y símbolos de Christoffel numéricos--- convierten la teoría en evidencia experimental: ahora puedes verificar, para tu familia favorita, que la geodésica $\alpha=1$ es recta en $\eta$ y la $\alpha=-1$ es recta en $p$, y observar empíricamente cómo la métrica de Fisher corrige el gradiente de la log-verosimilitud. Esa verificación numérica ---lado a lado con la teoría--- va a ser la prueba de consistencia que los ejercicios siguientes te pedirán completar a mano.
