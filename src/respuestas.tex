...
\begin{enumerate}
	...
	\item \textbf{[Softmax es exp-fam]} $p(x;\eta)=\prod\pi_i^{\mathbb{1}_{x=i}}=\exp(\eta^\top e_x - \log\sum e^{\eta_i})$. Identificamos $T(x)=\mathbf{e}_x\in\mathbb{R}^{K-1}$, $\eta=(\eta_1,\dots,\eta_{K-1})$ despreciando la redundancia $\sum\pi_i=1$, $A(\eta)=\log\sum_je^{\eta_j}$, $h(x)=1$.
