...
\bigskip
\noindent
Este capítulo abre el horizonte: lo que hemos construido hasta ahora ---Fisher, divergencias, familias exponenciales--- vive muy bien dentro de las distribuciones regulares; pero muchos modelos prácticos no son exponenciales. Cauchy, $t$-Student, mezclas pesadas de colas y Wasserstein son \emph{métricas distintas} sobre el mismo espacio, cada una útil en su contexto. Empezamos por el caso general ---variedades estadísticas sin estructura exponencial--- porque allí la métrica de Fisher sigue bien definida y eso basta para hablar con propiedad de distancia y covarianza.
...
\bigskip
\noindent
Isomap, LLE, t-SNE y UMAP ---los algoritmos de \emph{manifold learning}--- parecen herramientas de un cajón distinto; bajo el lente de la geometría de la información, son aproximaciones locales de geodésicas en la métrica de Fisher (o de una densidad subyacente). Esa reinterpretación explica por qué los grafos-\emph{$k$-NN} que construyen capturan ``intuitivamente la geometría: están trazando bordes del grafo de vecindad donde la métrica es alta. La siguiente generalización ---divergencias $\alpha$-$\beta$--- lleva la observación más lejos: hay un paraguas uniparamétrico que contiene KL, $f$-div y Bregman como casos extremos.

\section[Geometria de information alpha-beta]{Geometría de information $\alpha$-$\beta$}
...
\bigskip
\noindent
El transporte óptimo es geometría del \emph{espacio muestral}, no del espacio de distribuciones: dos histogramas sobre $\mathbb{R}^d$ se acercan tanto como cueste mover masa de uno al otro por la métrica euclidiana $\|x-y\|^2$. Esta noción ---más física que estadística--- explica por qué Wasserstein es preferible a KL cuando los soportes se tocan parcialmente o no se superponen: la KL diverge a $\infty$ si un píxel tiene probabilidad positiva en $p$ y nula en $q$; Wasserstein, no. La conexión con la geometría de la información reaparece cuando el espacio de parámetros de una red neuronal se ve, localmente, como una variedad estadística:
...
\bigskip
\noindent
Los dos cuadernillos Python ---el primero (\texttt{earth\_movers\_distance\_1d}) confirma cuándo Wasserstein detecta diferencias que KL ignora; el segundo (\texttt{gaussian\_geodesic}) verifica que las geodésicas en la familia Normal son rectas en coordenadas naturales $\eta$--- cierran el capítulo con dos ilustraciones que vale la pena ejecutar. Los ejercicios que siguen llevan esta observación a las $\alpha$-$\beta$-divergencias, los kernels característicos y la dinámica de entrenamiento de redes profundas.
...
\begin{enumerate}[resume]
	...
	\item[$\star\star\star$] Para un qubit puro $|\psi\rangle = \cos(\theta/2)|0\rangle + \sin(\theta/2)e^{i\phi}|1\rangle$, muestre que la métrica de Bures induce una distancia $d_B(|\psi\rangle, |\phi\rangle) = \arccos(|\langle\psi|\phi\rangle|)$. Calcule esta distancia para $|0\rangle$ y $(|0\rangle+|1\rangle)/\sqrt{2}$.
...
\begin{enumerate}[resume]
	...
	\item[$\star\star\star$] Para distribuciones discretas univariadas con soporte en $\{x_1 < x_2 < \cdots < x_k\}$, muestre que $W_1(p,q) = \sum_{i=1}^{k-1} |F_p(x_i) - F_q(x_i)| \cdot (x_{i+1} - x_i)$, donde $F$ es la función de distribución acumulada. Use esta fórmula para calcular $W_1$ entre $\mathrm{Uniforme}(0,1)$ y $\mathrm{Uniforme}(0.25,0.75)$ discretizadas en puntos $\{0, 0.25, 0.5, 0.75, 1\}$.
...
\begin{enumerate}[resume]
	...
	\item[$\star\star\star$] El gradiente natural requiere invertir la matriz de Fisher $I(\theta)$, lo cual cuesta $O(p^3)$ para $p$ parámetros. Proponga una aproximación diagonal $I(\theta) \approx \operatorname{diag}(I(\theta))$ que reduce el cómputo a $O(p)$. Compare la dirección del paso con la del gradiente estándar para un modelo Bernoulli con datos simulados.
	\item[$\star\star\star$] Proponga una divergencia híbrida que combine KL y Wasserstein: $D_\alpha(p \| q) = \alpha \cdot D_{\mathrm{KL}}(p \| q) + (1-\alpha) \cdot W_2^2(p,q)$ para $\alpha \in [0,1]$. ¿Qué ventajas tiene sobre KL pura cuando los soportes de $p$ y $q$ no se superponen completamente?
\end{enumerate}
