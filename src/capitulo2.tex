...
Las tres divergencias KL que hemos calculado ---entre dos Bernoulli paramétricas, dos Normales de igual varianza, dos Poisson--- responden a la misma fórmula abstracta $D_{\mathrm{KL}}(p\|q)=\int p\log(p/q)$, pero su estructura particular varía con cada familia: cuadrática en la Gaussiana, lineal-en-log en el parámetro de Poisson. La pregunta inmediata es si existe un paraguas que las abarque y muestre que \emph{desajuste} es un solo concepto con mil disfraces. La respuesta es sí: las \textbf{$f$-divergencias de Csiszár}, una sola función generadora $f$ parametrizará todas las divergencias ``convexas razonables'' en una fórmula común.
...
Esa ``uniformidad infinitesimal'' es un punto fuerte ---pero también una limitación: las $f$-divergencias viven familia por familia, sin un marco algebraico común que las conecte. La geometría de Bregman subsana esto: en lugar de elegir $f$ libremente, exigimos que la divergencia provenga de la aproximación lineal de una función \emph{estrictamente convexa} $F$. Veremos que, en ese caso, recuperamos KL como caso particular y emergen gratuitamente las nociones de dualidad de Legendre, las dos proyecciones alternativas y el teorema pitagórico.
...
La identidad dual anterior no es un ejercicio decorativo: anuncia que ``virar'' la divergencia equivale a invertir el orden de los argumentos al cambiar de coordenadas. Esta observación ---de apariencia inocente--- es el motor oculto del teorema pitagórico de la siguiente sección, porque reduce la comparación entre dos distribuciones al análisis en un sistema coordenado donde la convexidad de $A$ controla la ortogonalidad.
...
Hemos pasado revista a tres familias de divergencias (KL, $f$-div, Bregman), a dos dualidades (Legendre y m/e-proyecciones) y a la conexión profunda de la KL con la métrica de Fisher. Antes de cerrar el capítulo, conviene ver todo esto en movimiento ---calcular con Python cuánto vale cada divergencia entre dos distribuciones ajustadas, y comprobar empíricamente la identidad dual $D_F(p\|q)=D_{F^*}(\eta_q\|\eta_p)$ recién establecida.
...
kl_12 = kl_normal(mu1, s1, mu2, s2)
kl_21 = kl_normal(mu2, s2, mu1, s1)
# Result: KL(N1||N2) \approx  0.0453, KL(N2||N1) \approx  0.0548, asimetricas
\end{lstlisting}
...
q = np.array([0.5, 0.5])
bq = bregman_quad(p, q)
bk = bregman_neg_entropy(p, q)
# Result: Bregman cuadratica \approx  0.0400, Bregman KL \approx  0.1384
\end{lstlisting}
...
bregman_gap = F(p_val) - (F(q_val) + dF(q_val)*(p_val - q_val))
# Result: D_F(p||q) \approx  0.1738
\end{lstlisting}
...
\begin{enumerate}
	\item[$\star$] (V/F) La divergencia KL es siempre no negativa.
	\item[$\star$] (V/F) La divergencia KL es simétrica: $D_{\mathrm{KL}}(p \| q) = D_{\mathrm{KL}}(q \| p)$.
	\item[$\star$] (V/F) Toda $f$-divergencia satisface la desigualdad triangular.
	\item[$\star$] Defina la divergencia KL para distribuciones discretas.  ¿Qué significa que valga cero?
	\item[$\star\star$] Si $p = \mathrm{Bern}(0.8)$ y $q = \mathrm{Bern}(0.3)$, calcule $D_{\mathrm{KL}}(p \| q)$ y $D_{\mathrm{KL}}(q \| p)$.
\end{enumerate}

\subsection*{Cálculo de Divergencias}
\begin{enumerate}[resume]
	\item[$\star$] Calcule $D_{\mathrm{KL}}(\mathrm{Poisson}(3) \| \mathrm{Poisson}(5))$.
	\item[$\star$] Calcule $D_{\mathrm{KL}}(\mathrm{Poisson}(5) \| \mathrm{Poisson}(3))$.  ¿Es igual al anterior?
	\item[$\star\star\star$] Para la familia exponencial $\mathrm{Exp}(\lambda)$, muestre que $D_{\mathrm{KL}}(\mathrm{Exp}(\lambda_1) \| \mathrm{Exp}(\lambda_2)) = \log(\lambda_1/\lambda_2) + \lambda_2/\lambda_1 - 1$.
	\item[$\star$] Calcule la divergencia $\chi^2$ entre $\mathrm{Bern}(0.6)$ y $\mathrm{Bern}(0.4)$.
	\item[$\star$] Calcule la distancia de Hellinger entre $\mathrm{Bern}(0.7)$ y $\mathrm{Bern}(0.3)$.
	\item[$\star\star$] Verifique que $H^2(p,q) = 1 - \sum_x \sqrt{p(x)q(x)}$ es simétrica.
\end{enumerate}

\subsection*{Divergencias de Bregman}
\begin{enumerate}[resume]
	\item[$\star$] Para $F(p) = \frac{1}{2}p^2$ en $\mathbb{R}$, muestre que $D_F(p \| q) = \frac{1}{2}(p-q)^2$.
	\item[$\star$] Para $F(p) = p^2$, muestre que $D_F(p \| q) = (p-q)^2$.
	\item[$\star$] Para $F(p) = e^p$, calcule $D_F(p \| q)$.
	\item[$\star\star$] Encuentre la conjugada de Legendre $F^*$ de $F(p) = \frac{1}{2}p^TQp$ con $Q \succ 0$.
	\item[$\star\star\star$] Verifique la identidad dual $D_F(p \| q) = D_{F^*}(\eta_q \| \eta_p)$ para el caso cuadrático.
\end{enumerate}

\subsection*{Proyecciones y Teorema Pitagórico}
\begin{enumerate}[resume]
	\item[$\star\star$] Sea $\mathcal{C} = \{q : q_1 + q_2 = 1,\; q_i \ge 0\}$ (simplex de Bernoulli).  Si $p = (0.6, 0.4)$ y $\mathcal{C}$ se reduce al punto $q^\star = (0.5, 0.5)$, verifique el teorema pitagórico.
	\item[$\star\star\star$] Para una cadena de Markov $X \to Y \to Z$, muestre que $D_{\mathrm{KL}}(p_Z \| q_Z) \le D_{\mathrm{KL}}(p_X \| q_X)$ (pérdida por procesamiento de datos).
	\item[$\star\star\star$] Demuestre que la $m$-proyección de $p$ sobre el conjunto de distribuciones con esperanza fija $\mu$ en una familia exponencial es la distribución exponencial con ese momento.
\end{enumerate}

\subsection*{Propiedades Teóricas}
\begin{enumerate}[resume]
	\item[$\star\star$] Demuestre que $-\log t \le 1 - t$ para $t > 0$ (usado en la prueba de Gibbs).
	\item[$\star\star\star$] Muestre que si $f$ es convexa con $f(1)=0$, entonces $I_f(p \| q) \ge 0$.
	\item[$\star\star\star$] Para la KL, muestre que $\nabla_\theta D_{\mathrm{KL}}(p_\theta \| q) \big|_{\theta=\theta_0} = 0$ cuando $q = p_{\theta_0}$.
	\item[$\star\star$] Demuestre que la divergencia KL \emph{no} satisface la desigualdad triangular: dé un contraejemplo con Bernoulli.
	\item[$\star\star\star\star$] (Reto) Muestre que toda $f$-divergencia es invariante bajo transformaciones biyectivas medibles del espacio muestral.
\end{enumerate}

\subsection*{Qué gana con GI}
\begin{quote}
	\itshape
	Sin GI, las divergencias se estudian como herramientas aisladas para medir desajuste.  Con GI, se entienden como \emph{distancias} en un espacio geométrico: la KL es la divergencia de Bregman generada por la entropía negativa, y el teorema pitagórico garantiza que la proyección KL sobre una familia constrañida descompone el error en dos términos ortogonales.  Esto explica por qué el algoritmo EM converge: cada paso es una proyección alternada que reduce la KL en al menos un término del pitagórico.
\end{quote}
