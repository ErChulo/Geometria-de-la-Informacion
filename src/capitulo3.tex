...
Los tres ejemplos ---Bernoulli, Normal y Poisson--- exhiben una coincidencia estructural sospechosa: el término exponencial de la densidad se factoriza como $\eta\!\cdot T(x)-A(\eta)$, donde $\eta$ no es necesariamente el parámetro ``natural'' con el que uno llegó al modelo (es $\log(p/(1-p))$ para Bernoulli, $\mu/\sigma^2$ para la Normal media-desconocida, $\log\lambda$ para Poisson). La coordenada $\eta$ revela una geometría oculta: es la variable bajo la cual la función de partición $A$ resulta convexa, y la divergencia KL se reduce a una Bregman canónica. Falta entonces precisar la relación entre $\eta$ y el parámetro ``humano'' $p$, $\mu$, $\lambda$.
...
Con la dualidad de Legendre establecida ---$\mu=\nabla A(\eta)$ y $\eta=\nabla A^*(\mu)$---, las coordenadas $\eta$ y $\mu$ son dos caras de la misma moneda convexa. Falta ahora el cálculo que motivó toda esta maquinaria: ¿cuánto vale la divergencia KL entre dos distribuciones $p_{\eta_1}$ y $p_{\eta_2}$ de la misma familia exponencial? El resultado es elegante, y un poco desconcertante la primera vez: la KL entre distribuciones equivale a una divergencia de Bregman entre \emph{coordenadas naturales}, pero los subíndices se invierten al cruzar la transformada de Legendre.
...
\begin{theorem}[KL en la familia exponencial]
	...
	\]Esto es exactamente la divergencia de Bregman generada por $A$, con un cruce crucial de subíndices:
	\[
	D_{\mathrm{KL}}(p_{\eta_1} \| p_{\eta_2}) \;=\; D_A(\eta_2 \| \eta_1).
	\]
	\begin{remark}[Por qué se invierten los subíndices]
		La asimetría no es error tipográfico: $D_{\mathrm{KL}}$ vive en el espacio de distribuciones y $D_A$ vive en el espacio de coordenadas naturales $\eta$. La transformada de Legendre $\eta \mapsto \mu = \nabla A(\eta)$ ``gira'' el orden al pasar del segundo al primero. Regla nemotécnica: \emph{``$D_{\mathrm{KL}}$ de $p_{\eta_1}$ a $p_{\eta_2}$ equivale a $D_A$ de $\eta_2$ a $\eta_1$''}. Compara letra por letra con el corolario siguiente y verás que el cómputo cierra sin truco.
	\end{remark}
\end{theorem}
...
\begin{corollary}[Asimetría de la KL]
	La divergencia KL es asimétrica en general: $D_{\mathrm{KL}}(p_{\eta_1} \| p_{\eta_2}) = D_A(\eta_2 \| \eta_1)$ y $D_{\mathrm{KL}}(p_{\eta_2} \| p_{\eta_1}) = D_A(\eta_1 \| \eta_2)$, y estas son distintas porque $D_A$ no es simétrica.
\end{corollary}
...
\begin{center}
	\begin{tikzpicture}
		\begin{axis}[
			...
			\draw[Accent, thick] (E1) -- (E2) node[midway, above] {$D_A(\eta_2\|\eta_1)$};
		\end{axis}
	\end{tikzpicture}
\end{center}
...
\begin{example}[Normal: geometría dual]
	...
	\begin{itemize}
		...
		\item La transformada de Legendre: $A(\eta) = \sigma^2 \eta^2/2$, $A^*(\mu) = \mu^2/(2\sigma^2)$.\item $D_{\mathrm{KL}}(\mathcal{N}(\mu_1,\sigma^2) \| \mathcal{N}(\mu_2,\sigma^2)) = (\mu_1-\mu_2)^2/(2\sigma^2)$, que es cuadrática en las coordenadas naturales.
\end{itemize}
\end{example}

Las familias exponenciales son entonces \emph{dualmente planas}: las geodésicas son rectas en $\eta$ y en $\mu$, y la métrica de Fisher ($g_{ij}=\nabla^2 A(\eta)$, el Hessiano de $A$) es plana. La tentación es pensar que esta planitud es genérica. El teorema de Pitman--Koopman--Darmois ---que enunciamos y demostramos en la próxima sección--- revela lo opuesto: bajo condiciones muy razonables (suficiencia de dimensión fija e identificabilidad), las familias exponenciales son las \emph{únicas} con esta geometría dual plana. La planitud, lejos de ser trivial, es una rareza algebraica.
...
La tabla anterior debería dejar una huella: casi todos los modelos que un estadístico o un ingeniero de machine learning maneja ---ocho de trece--- son familias exponenciales, y los que no lo son (Cauchy, $t$-Student, Uniforme $U(0,\theta)$, Weibull, log-normal) comparten una característica común: carecen de estadístico suficiente de dimensión fija en el sentido preciso de Pitman--Koopman--Darmois (recordemos §6: el máximo de $n$ uniformes $[0,\theta]$ es suficiente, pero la densidad no se factoriza en la forma exponencial). Verificar mecánicamente que tu modelo favorito es exponencial es, por tanto, una destreza práctica que conviene ejercitar. El siguiente código Python ofrece esa verificación ---factorizando la densidad en la forma $\eta\!\cdot T(x)-A(\eta)$--- y luego calcula la KL entre dos distribuciones de la misma familia reescribiéndola como Bregman sobre $A$, con el cruce de subíndices característico.
...
def kl_as_bregman(eta1, eta2, dist_name):
	"""Calcula D_KL entre p_{eta1} y p_{eta2} como D_A(eta2 || eta1) en coords naturales.
	El orden de los subíndices se invierte al pasar de distribuciones a coordenadas naturales (ver Cap. 3)."""
