...
Para anclar la fórmula en distribuciones concretas, veamos cuatro casos en los que la métrica captura intuiciones diferentes: en Bernoulli la información explota cerca de los bordes $p\approx 0$ o $p\approx 1$; en Poisson decae como $1/\lambda$ y en Exponencial como $1/\lambda^2$. El patrón general: distribuciones más \emph{concentradas} son más distinguibles; distribuciones más \emph{dispersas} se parecen entre sí.
...
La figura anterior muestra cómo la información de Fisher se concentra en los bordes de la familia Bernoulli. Pero un solo número $I(p)$ en un punto no basta: para medir \emph{distancia} entre dos distribuciones necesitamos que Fisher defina un producto interno completo en el espacio tangente de la variedad. Eso es lo que sigue.
...
\section[El caso bivariante: Normal (mu, sigma)]{El caso bivariante: Normal $(\mu, \sigma)$}
...
Las cuatro familias de los ejemplos y la sorpresa hiperbólica del caso bivariante muestran que la fórmula \emph{funciona}. Falta explicar por qué funciona \emph{ESTA} fórmula y no cualquier otra. La respuesta viene de un teorema de unicidad ---y revela que la métrica de Fisher no se elige: se impone.
...
Y tiene una consecuencia práctica inmediata: si la métrica no depende de la parametrización elegida, tampoco deberían depender de ella las cotas inferiores sobre la varianza de cualquier estimador insesgado. Es exactamente lo que afirma la siguiente sección.
...
\begin{enumerate}[resume]
	...
	\item[$\star\star\star$] \textbf{Softmax es exponencial.} Muestre que la categórica $K$-aria $\mathrm{Categorical}(\pi_1,\dots,\pi_K)$ con $\pi_i=e^{\eta_i}/\sum_j e^{\eta_j}$ es familia exponencial con \\
$\eta=(\eta_1,\dots,\eta_{K-1})^T$,\ $T(x)=(e_{x_1},\dots,e_{x_{K-1}})^T$,\ $A(\eta)=\log\sum_j e^{\eta_j}$.
