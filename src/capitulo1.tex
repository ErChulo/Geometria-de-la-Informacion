...
\begin{enumerate}[resume]
	...
	\item[$\star\star\star$] \textbf{Softmax es exponencial.} Muestre que la categórica $K$-aria $\mathrm{Categorical}(\pi_1,\dots,\pi_K)$ con $\pi_i=e^{\eta_i}/\sum_j e^{\eta_j}$ es familia exponencial con $\eta=(\eta_1,\dots,\eta_{K-1})^T$, $T(x)=(e_x_1,\dots,e_x_{K-1})^T$, $A(\eta)=\log\sum_j e^{\eta_j}$.
