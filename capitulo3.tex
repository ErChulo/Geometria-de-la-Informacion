...
print(f"Bernoulli(0.7): eta={bern_decomp['eta']:.4f}, "
      f"psi={bern_decomp['psi']:.4f}")

pois_decomp = exponential_family_decomposition("poisson", {"lambda": 3.0})
print(f"Poisson(3): eta={pois_decomp['eta']:.4f}, "
      f"psi={pois_decomp['psi']:.4f}")
\end{lstlisting}
...
print(f"KL via Bregman:  {kl_bregman:.4f}")
print(f"KL directa:      {kl_direct:.4f}")
\end{lstlisting}
...
\begin{enumerate}
	\item Verdadero o falso: Toda familia exponencial tiene un estadístico suficiente de dimensión finita.
	\item Verdad o falso: La divergencia KL entre dos distribuciones de una familia exponencial es simétrica.
	\item ¿Cuál es la relación entre los parámetros de expectación $\mu$ y los parámetros naturales $\eta$?
	\item Muestre que la familia de Cauchy con localización y escala desconocidas \emph{no} es exponencial.
	\item ¿Qué propiedad de la familia exponencial garantiza que la función de partición $A(\eta)$ sea convexa?
\end{enumerate}

\subsection*{Cálculos con familias exponenciales}
\begin{enumerate}[resume]
	\item Para $X \sim \mathrm{Bern}(p)$, encuentre: (a) $\eta$, (b) $A(\eta)$, (c) $\mu = \nabla A(\eta)$.
	\item Para $X \sim \mathrm{Poisson}(\lambda)$, encuentre: (a) $\eta$, (b) $A(\eta)$, (c) $\mu$.
	\item Para $X \sim \mathcal{N}(\mu, 1)$ con $\mu$ desconocido, encuentre $\eta$, $A(\eta)$ y $A^*(\mu)$.
	\item Calcule $D_{\mathrm{KL}}(\mathrm{Bern}(0.6) \| \mathrm{Bern}(0.4))$ usando la fórmula $D_A(\eta_1 \| \eta_2)$.
	\item Calcule $D_{\mathrm{KL}}(\mathrm{Poisson}(2) \| \mathrm{Poisson}(4))$ usando la fórmula $D_A$.
	\item Verifique la igualdad $D_{\mathrm{KL}}(p_{\eta_1} \| p_{\eta_2}) = A(\eta_2) - A(\eta_1) - \langle \eta_2 - \eta_1, \nabla A(\eta_1) \rangle$ para $\mathrm{Bern}(0.7)$ vs $\mathrm{Bern}(0.3)$.
\end{enumerate}

\subsection*{Dualidad de Legendre}
\begin{enumerate}[resume]
	\item Para $\mathrm{Poisson}(\lambda)$, verifique que $\nabla A^*(\mu) = \eta$.
	\item Para $\mathrm{Bern}(p)$, verifique que $A^*(\mu) = \mu\log(\mu/(1-\mu)) + \log(1-\mu)$.
	\item Para $\mathcal{N}(\mu, \sigma^2)$ con $\sigma^2$ fijo, muestre que $A^*(\mu) = \mu^2/(2\sigma^2)$.
	\item Demuestre que la transformada de Legendre es involutiva: $(A^*)^* = A$.
	\item Muestre que $A(\eta) + A^*(\mu) = \langle \eta, \mu \rangle$ en el punto duale.
\end{enumerate}

\subsection*{Teorema de Pitman--Koopman--Darmois}
\begin{enumerate}[resume]
	\item Demuestre que la familia $U(0, \theta)$ no tiene un estadístico suficiente de dimensión finita.
	\item Muestre que la familia de Cauchy con parámetro de localización $\theta$ no es exponencial.
	\item Verifique que la familia exponencial $\mathrm{Exp}(\lambda)$ satisface las hipótesis del PKD.
	\item Dé un ejemplo de una familia no exponencial donde la suficiencia es ``infinita''.
	\item (Reto) Demuestre que si una familia es exponencial y tiene un solo parámetro, entonces $D_{\mathrm{KL}}(p_{\theta_1} \| p_{\theta_2})$ es estrictamente convexa en $\theta_1$ para $\theta_2$ fijo.
\end{enumerate}
